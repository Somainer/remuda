...
│                             │          │ ≥60% 视口（键盘收起）                     │
├─────────────────────────────┤          ├────────────────────────────────────────┤
│ 会话 收件箱·n 新建 更多 56   │          │ composer 56／本地输入 44               │
└─────────────────────────────┘          │ ＋键盘条 44（仅终端段）                │
                                         │ （无 app 底栏、无 tab 行）             │
                                         └────────────────────────────────────────┘
```
...
- 会话路由只有一条 52px 页头，从左到右：返回 `‹`（字形 20px，热区 44×44）、标题块、`终端|结构` 分段（可见 26px，每项热区 44）、Stop（32px 方块，`::after` 热区 44）、⋯（32px，热区 44）。标题块兼任 Space 抽屉触发（`spaces-drawer-open`）：第一行是状态点 + 标题，第二行是 `spaces-chips` 里的 Space 名 + 状态词；D-049 的截断三步逐字保留（§4.7）。
- 该路由的「底部条」由 composer（结构段，收起态 56px，D-042）或本地输入条 + 键盘条（终端段，各 44px，§2.3）担任，app 级底栏（`nav[aria-label="手机底栏"]`）不渲染，SpaceTabs 行也不渲染。切空间由标题块的抽屉与 Jump To sheet 承担，能力不降级。
...
- **面板选中态与已退出分组**：当前 Space 与当前会话都用 `--bg-selected` 底 + `--fg-strong` 字 + 加粗标题，两者都能一眼看出，深浅两态都有足够对比、不单靠颜色。每个 space 下的「已退出 (n)」分组默认折叠，行内提供 **恢复**（既有 resume 能力）与 **删除**（`DELETE /v1/instances/{id}`，确认「删除会话及其记录？」）。运行中会话按钮为「停止并删除」，走 `?force=1` 由 Hub 停止并删除，客户端不再自行先 close；404 按幂等成功处理，`nodePurge` 非 `purged` 时提示主机侧数据待清理。删除失败保留该行并提示，不显示成功文案。
- **活动 tab** 用 2px 下划线 + `--bg-selected` 底 + 加粗；键盘焦点环沿用全局 `:focus-visible`。
...
呈现顺序，从左到右：6px 圆点 → 阶段词（`live-phase`）→ 工具名（最宽 180px，手机 120px）→ elapsed（`live-elapsed`）→ `↓ 1.2k`（`live-token-count`）→ phrase（`live-phrase`，最先省略）。最右侧是 tier 与 decided-by 两段纯文字，以及健康提示 token「hook 静默 / hook 无记录」，用 `--attention-fg`；testid `live-health-<tier>` 和 `data-reason` 保留。宽度 <480px 时先去掉 token 计数和 tier。`live-interrupt` 的去留：先验证「凡是 `canInterrupt` 为真时，composer 的 `controls.interrupt.available` 也为真」，验证通过就删除；不通过就在行尾保留一个安静的「打断」文字按钮。
...
compact（手机）下 control bar 是单行 56px：`[选项触发器] [文本框 flex 1] [插队] [打断] [发送]`。

- **选项触发器**（`composer-options-trigger` / `model-effort-chip`）：可见 32px，13px 字，最宽 132px，显示「manual · high ▾」，热区 44；`danger` 模式时 `--danger-fg` 字、⚠ 前缀、1px `--danger-border`。触发器**必须同时显示两样东西**：当前 `permissionMode` 的**词**（`manual` / `acceptEdits` / `dontAsk` / `bypassPermissions` 的 label 或 native 词）**和** effort 的**档名**（上一条「只显示档名」的要求不因收起而豁免），形如 `manual · high`。`permissions.ts` 标为 `danger` 的模式（绕过全部 / 不再询问 / 完全访问）必须在触发器上就用 danger 样式可见——**把 bypass 态藏进 sheet 是本条最响的禁止项**。
- **插队、打断**：只在忙碌时出现。都是 32px 的 `.iconBtn`，热区 44，间距 12；aria-label 分别为「插队发送」「打断」；testid 分别为 `composer-steer`、`composer-interrupt`。**发送**：可见 32px 的圆，热区 44。
...
- 桌面工具条高 32px；TUI 模式标识 12px 胶囊；过期提示「⚠ 画面可能过期」用 `--attention-fg`；进度条 2px。
- 辅助键条 `AuxKeys`：按键高 28px，12px 等宽字。
- 本地输入 `LocalInput`：输入框高 36px；字号用 `--text-input`——coarse 指针下 16px（不再是 15px），提示文字 12px（不再是 10.5px）；手机键盘条 `PhoneKeyBar`：按键 44×44，13px 等宽字，整条横向滚动。
...
- **两侧档位各自保留，不强制一致**：桌面保留三档（待你处理 / 进行中·最近 / **已离队**——即上面线框的「过期」行与 superseded / paused 行）+ 主机 / Workspace 过滤芯片（query `host` / `workspace`）；compact 保留两档（待你处理 / 进行中·最近，**没有**已离队第三档，`web/src/features/mobile/inboxRows.ts` 的既有决定）。档位组成由 `mode` prop 决定，**不是**静默删掉任一方行为。D-049「投影只有一份」约束壳与卡控，不强制两面的信息架构相同（两面线框本来就不同，§4.7）。
...
- 每行最小高 44px；派生 **`SE-nn`** 人读 key（project 内顺序派生，display-only，无 schema 改动；不显示裸 `tsk_…`）、标题、session 数、一句下一步；归档 task 折进「已归档 · N」。选中行用 `--bg-selected` 底，加 2px `--focus` 左竖线。
...
- **会话路由 `/s/:id*` 在 compact 下不渲染 app 底栏、也不渲染 tab 行**（§1.3 / §1.4）：结构段底部只有收起态 composer（56px，D-042），终端段底部是本地输入条 44px + 键盘条 44px（§2.3）。顶栏返回键字形 20px、热区 ≥ `--touch`（§3.4）。
- **正文（transcript 或 xterm）在 composer 收起、无软键盘时，可视高度 ≥ 视口高的 60%。** 结构段测 `data-testid="session-body"`，终端段测 xterm 容器。390×844 的预算余量：
  ...
  | 终端 | 安全区 47 + 页头 52 + 本地输入 44 + 键盘条 44 + 底部 34 | 623px | 74% |

  **60% 是验收下限，不是设计目标。**
- 触控一律走热区（§3.4）：视觉字形不变，`::after` 或 padding 撑到 `--touch`，热区不得互相重叠。
