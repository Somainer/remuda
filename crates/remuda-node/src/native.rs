    ...
    fn start(&self) -> DriverStartFuture<'_> {
        Box::pin(async move {
            ...
            if let Ok(mut slot) = self.startup_error.lock() {
                // D-036 / model-pin-1: a pin that reached no channel is a
                // refusal, not a launch. Reported as a startup error so the
                // runtime's existing path tears the process down and fails the
                // instance rather than leaving it running on the wrong model.
                *slot = handle
                    .ack()
                    .native_ids
                    .get("lastError")
                    .cloned()
                    .or_else(|| model_pin_channel_error(&self.spec, handle.recipe()));
            }
...
/// Lifecycle error name for a launch whose model pin reached no channel.
pub const MODEL_PIN_UNREACHABLE: &str = "model-pin-unreachable";

/// Refuse a launch whose explicit model pin reached none of its channels
/// (D-036 / model-pin-1, brief step 4).
///
/// Why this shape rather than comparing the pin to the first observed
/// `message.model`, as the brief sketched: measured on a gateway-configured host
/// (2026-09-18, `docs/design/evidence/model-pin-1.md` §3), a *correctly* pinned
/// session records an upstream resolution there, not the pin — a session pinned
/// to `model_hub/es1_orange_o50[1m]` writes `message.model = claude-opus-5`.
/// Neither that id nor the host-default id it would be confused with appears in
/// the gateway catalog, so assistant records cannot separate "the pin resolved"
/// from "the host default answered". Gating a refusal on them would stop nearly
/// every correct launch while still missing the substitution it was written for.
/// The codebase already says so: [`remuda_protocol::ModelTracker::note_stdout`]
/// settles an awaiting pin on the verdict regardless of id equality, because
/// "the requested alias resolved to this concrete id".
///
/// What *is* decidable at launch, and is exactly the 2026-09-18 failure: whether
/// the pin reached the process at all. A pin with no `--model` argv and no
/// overlay `model` key cannot be in effect, whatever the transcript later says.
/// That check is local, deterministic, and needs no network or model catalog.
///
/// A launch with no pin cannot mismatch and is never refused (rule 3).
fn model_pin_channel_error(
    spec: &InstanceSpec,
    recipe: &remuda_driver::LaunchRecipe,
) -> Option<String> {
    let pin = remuda_driver::pinned_model_for(spec)?;

    // Channel 1: the argv token, byte-identical to the pin.
    let on_argv = recipe
        .argv
        .windows(2)
        .any(|pair| pair[0] == "--model" && pair[1] == pin);
    if on_argv {
        return None;
    }
    // Channel 2: the merged settings overlay's `model` key. Read from the
    // audited materialized file so this checks the bytes the CLI was handed,
    // not what we believe we wrote.
    let on_overlay = recipe
        .materialized_files
        .iter()
        .filter(|file| file.role == remuda_driver::FileRole::Settings)
        .any(|file| overlay_model(&file.path).as_deref() == Some(pin.as_str()));
    if on_overlay {
        return None;
    }
    Some(format!(
        "{MODEL_PIN_UNREACHABLE}: requested {pin} but the launch carries no --model argv \
         and no settings model key, so the host's default would answer"
    ))
}

/// The `model` key of a settings overlay on disk, when it has a usable one.
fn overlay_model(path: &str) -> Option<String> {
...
mod tests {
    ...
    /// D-036 / model-pin-1: the launch-channel refusal.
    mod model_pin {
        use super::*;
        ...
        /// A recipe in the exact shape the 2026-09-18 demo produced: only
        /// `--settings`, no model token anywhere.
        fn recipe_with(argv: Vec<String>, settings: Option<&Path>) -> remuda_driver::LaunchRecipe {
            let mut recipe: remuda_driver::LaunchRecipe =
                serde_json::from_value(serde_json::json!({
                    "launchId": "launch_01993ab0-0000-7000-8000-00000000000a",
                    "driver": "shell-pty",
                    "binary": {
                        "absPath": "/opt/claude/bin/claude",
                        "version": "2.1.272",
                    ...
                    "inputDelivery": "tty",
                    "provider": {
                        "profileId": "pvp_01993ab0-0000-7000-8000-000000000001",
                        "kind": "anthropic",
                        "baseUrl": "",
                        "delegation": "none",
                        "secretRef": null,
                    ...
                    "permission": {"cliMode": null, "prompts": null, "extraFlags": []},
                    "technicalDebt": [],
                    "audit": {
                        "envNames": [],
                        "credentialRefs": [],
            ...
            if let Some(path) = settings {
                recipe
                    ...
                    .push(remuda_driver::MaterializedFile {
                        ...
                        role: remuda_driver::FileRole::Settings,
                        mode: "0600".into(),
                        content_digest: format!("sha256:{}", "0".repeat(64))
                            .try_into()
                            .expect("digest"),
                        lifetime: remuda_driver::FileLifetime::Launch,
                    });
            }
            recipe
        }

        fn spec_with_pin(pin: Option<&str>) -> InstanceSpec {
            ...
            spec.model_id = pin.map(str::to_owned);
            spec
        }

        /// The regression itself: a pin, and a launch carrying neither channel.
        #[test]
        fn a_pin_that_reached_no_channel_is_refused_naming_both_ids() {
            let recipe = recipe_with(
                vec!["--settings".into(), "/instance/launch/settings.json".into()],
                None,
            );
            let error = model_pin_channel_error(&spec_with_pin(Some(PIN)), &recipe)
                .expect("a pin with no channel must refuse");
            assert!(error.contains(MODEL_PIN_UNREACHABLE), "{error}");
            // The requested id is named, verbatim suffix included, so the
            // operator can see which pin was dropped.
            assert!(error.contains(PIN), "{error}");
        }

        /// The argv channel satisfies the pin.
        #[test]
        fn the_argv_channel_is_accepted() {
            let recipe = recipe_with(vec!["--model".into(), PIN.into()], None);
            assert_eq!(
                model_pin_channel_error(&spec_with_pin(Some(PIN)), &recipe),
                None
            );
        }

        /// A near-miss on argv is not the pin: a suffix-stripped id selects the
        /// other context variant, so it must not satisfy the check.
        #[test]
        fn a_suffix_stripped_argv_value_does_not_satisfy_the_pin() {
            let recipe = recipe_with(
                vec!["--model".into(), "model_hub/es1_orange_o50".into()],
                None,
            );
            assert!(model_pin_channel_error(&spec_with_pin(Some(PIN)), &recipe).is_some());
        }

        /// The overlay channel satisfies the pin on its own (belt and braces).
        #[test]
        fn the_overlay_channel_is_accepted() {
            let tmp = tempfile::tempdir().unwrap();
            let overlay = tmp.path().join("settings.json");
            std::fs::write(&overlay, serde_json::json!({"model": PIN}).to_string()).unwrap();
            let recipe = recipe_with(
                vec!["--settings".into(), overlay.to_string_lossy().into_owned()],
                Some(&overlay),
            );
            assert_eq!(
                model_pin_channel_error(&spec_with_pin(Some(PIN)), &recipe),
        ...
        /// An overlay naming the *host's* model is the demo bug with a settings
        /// file attached: it must still refuse.
        #[test]
        fn an_overlay_carrying_the_host_model_is_still_a_refusal() {
            let tmp = tempfile::tempdir().unwrap();
            let overlay = tmp.path().join("settings.json");
            std::fs::write(
                &overlay,
                serde_json::json!({"model": HOST_DEFAULT}).to_string(),
            )
            .unwrap();
            let recipe = recipe_with(
                vec!["--settings".into(), overlay.to_string_lossy().into_owned()],
            ...
            let error = model_pin_channel_error(&spec_with_pin(Some(PIN)), &recipe)
                .expect("host model is not the pin");
            assert!(error.contains(PIN), "{error}");
        }

        /// Rule 3 / "a pin that was never requested cannot mismatch": no pin is
        /// silent, even with no model token anywhere.
        #[test]
        fn a_launch_without_a_pin_is_never_refused() {
            let recipe = recipe_with(vec!["--settings".into(), "/x/settings.json".into()], None);
            assert_eq!(model_pin_channel_error(&spec_with_pin(None), &recipe), None);
            assert_eq!(
                model_pin_channel_error(&spec_with_pin(Some("   ")), &recipe),
                None
            );
        }
    }
