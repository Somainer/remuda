...
- The async producers pre-fold on `spawn_blocking` before taking the mapper
  lock (live stdout, pty transcript pump) and the subagent/workflow
  transcript tails carry the same stager slot; staging parks on a bounded
  (95 s) dedicated thread, never on a tokio worker.
...
- Full hub suite (serial, same ports), final post-review tree:
  `122 passed, 16 skipped, 0 failed` (17.8 min). An earlier load-heavy run
  had one unrelated evidence-screenshot flake
  (`ux-code.hub.spec.ts` element detached while capturing the 390px evidence
  image); that file passed 2/2 alone and the final run was clean.
...
subagent/workflow screenshots are staged on those tails (this change), a
screenshot-bearing session retains megabyte-scale raw lines for the object
lifetime. Filed as a follow-up rather than widened in this task.

## Unit coverage

- `remuda-protocol/src/tool_media.rs` — text/image/mixed folding, encoded-
  length pre-check, per-result cap, pre-folded marker, unknown-item notes,
  missing stager and malformed/over-size/unstageable fallbacks, sidecar
  scrubbing (including idempotency).
