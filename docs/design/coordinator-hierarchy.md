...
| **6** | `co-lanes` | `crates/remuda/src/cmd/{gate.rs,land.rs,report.rs}`(新)；`crates/remuda-hub/src/gatequeue.rs`(新)；`crates/remuda/src/cmd/dispatcher.rs` + `crates/remuda-feishu/src/dispatcher.rs`（intake 改道 + `/project`）；`crates/remuda-feishu/src/cards.rs`（三类卡片）；`web/src/lib/api.generated.ts`（重生成） | `crates/remuda/tests/land_cli.rs`(新)：两 lane 并行验证 + 全局 land 锁 + **合成 BaseMoved**（land 期间 main 前进 → 退出码 `BaseMoved` → 重验一次）、非祖先 sha 一律拒绝、lane 的 port block 与 target dir 由产品分配且互不相撞；`crates/remuda-feishu/tests/` 增补：一次 intake→placement 卡→outcome 卡的端到端（fake node） | `docs/design/evidence/coord-lanes-1.md` | L | `r-mergequeue` 合入 **且** 批 5 合入后 → **M2** |
