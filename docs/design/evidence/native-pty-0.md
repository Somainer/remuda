...
> **⚠ 这是一处与设计文档的偏离，需要协调者确认。** §4.1 与 D-028 决策条目正文都
> 写的是 `wezterm-term`；本实现选了 `vt100`，理由见下。设计文档 §4.1 第一行与
> decisions.md 的 D-028 条目里那句「`wezterm-term` 模拟器」应相应更新——那两个文件
> 不在本 worker 的 ownership 内，故未改动，在此标出。
...
**重绘不总是更小，这一点如实说明**：在一个只输出了 21 字节的空会话上，重绘是
56 字节（reset + 模式集比内容还长）。重绘省的不是常数开销，而是**与会话长度解耦**
——它的大小只取决于当前屏幕，ring 的大小取决于会话跑了多久，上限 256 KiB。
...
`origin/main` 的干净 worktree 做对照（对照当时 main 为 `7cecd44`），**两边逐字相同**：
...
3–4 s 通过，rebase 前后各验证过一次）。修它需要改 macOS 系统设置，这是本 worker
明令禁止的操作，故**未修，如实记录**。本改动新增的 48 个 Rust 测试与 6 个 web
测试全部通过。
