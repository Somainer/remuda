...
export function SpacesPanel({ spaces, active, prefs, instanceId, onSelect, onNavigate, drawer = false }: {
  spaces: Space[]; active?: Space; prefs: SpacePrefs; instanceId?: string; onSelect: (space: Space) => void;
  onNavigate?: () => void;
  /**
   * Accepted from the /sessions index caller; the panel no longer has a rail
   * or its own fold — folding is the sidebar's concept (⌘/Ctrl+B, D-053).
   */
  collapsed?: boolean;
  drawer?: boolean;
}) {
  ...
  return <section className={css.panel} data-testid="spaces-panel" aria-label="空间与会话">
    <header className={css.panelHead}>
      <span className={css.panelTitle}>空间</span>
    </header>
    <div className={qfCss.panelSearch}>
      <QuickFindTrigger />
    </div>
    <div className={css.groups}>
      {spaces.map((space, index) => {
        ...
        return <section key={space.id} className={css.group} data-active={space.id === active?.id}>
          <div className={css.groupHead}>
            <button type="button" className={css.disclosure} aria-label={`${prefs.groupCollapsed[space.id] ? "展开" : "折叠"} ${space.name}`} aria-expanded={!prefs.groupCollapsed[space.id]} onClick={() => spaceStore.toggleGroup(space.id)}>{prefs.groupCollapsed[space.id] ? "▸" : "▾"}</button>
            <button type="button" className={css.spaceSelect} data-testid="space-select" data-space-id={space.id} aria-pressed={space.id === active?.id} title={`${space.name} · ${space.liveCount} 活跃 · ${space.blockedCount} 待处理`} onClick={() => onSelect(space)}>
              <span className={css.spaceName}>{space.name}</span><span className={css.count} aria-label={`${space.liveCount} 活跃，${space.blockedCount} 待处理`}>{space.liveCount}<span className={space.blockedCount ? css.blocked : ""}> / {space.blockedCount}</span></span>
            </button>
          </div>
          {!prefs.groupCollapsed[space.id] ? <>
            ...
            }}><input className={css.renameInput} name="name" aria-label="空间名称" defaultValue={space.name} autoFocus maxLength={80} onKeyDown={(event) => { if (event.key === "Escape") setRenaming(undefined); }} /><button type="submit" className={css.tool}>保存</button></form> : null}
      ...
      {!spaces.length ? <p className={css.empty}>注册工作区后在这里切换项目</p> : null}
    </div>
    <footer className={css.panelFoot}><span>⌘/Ctrl+[ ] 切换空间</span></footer>
    {/*
      The cross-space finder is always mounted: its global ⌘/Ctrl+K listener
      has to work from a session page next to an attached terminal, and the
      keyboard-scope guard keeps it out of the PTY bytes. On a phone the panel
      lives in the Spaces drawer, which is the same entry.
    */}
