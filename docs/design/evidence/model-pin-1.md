...
| `remuda-hub/tests/hub.rs::model_pin_mismatch_is_served_on_public_instance_json` | end-to-end over the authenticated HTTP API: create → Node-appended `model_pin_mismatch` → GET `/v1/instances/{id}` returns exact `modelPinMismatches` JSON (requested/observed/observedAt), independent of the bounded journal tail |
...
| `web/.../EffortSlider.test.tsx` | post-§6/§7: the collapsed chip shows the single RUNNING model verbatim (read-back id, else durable launch spec) for claude AND codex, nothing when a model-axis launch has neither value, and keeps the tier stop description for effort-only sliders (no `model` prop) — never a two-id pair |
...
| `web/src/features/session/SessionList.test.tsx` | post-§6: the row chip shows the one running id verbatim (read-back else launch spec), omits the slot entirely when neither exists — never a two-id pair |
