...
//! human approvals. Grants disappear on Hub restart (fail closed). SPEC-ONLY (unimplemented, 2026-09-22 c-nodecosign): origins stamped by `stamp` are authoritative only *inside* the Hub — the Node trusts the same field from the frame alone (`crates/remuda-node/src/origin.rs:25-32`; `restrict_permission` below reads it at the line cited by `docs/design/evidence/node-cosign-1.md` §1). `docs/design/passkey-login.md` §6 specifies the Node-side device-passkey co-signature: privileged frames missing a valid device assertion refuse before dispatch, never downgrade (D-035 shape). The Hub-side checks in this module do not move.
