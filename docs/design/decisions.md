...
| D-034 | 2026-09-17 | **`claude-print` 降级为「显式指定的诊断 carrier」，禁止作为任何默认或回落值**（owner 增补：print 作为 worker carrier 无用——一个 session 跑完一轮就结束，必须手工 resume）。**决策**：(1) **Hub** `driver_for` 的默认顺序改为「宿主 `driverInventory` 报告 `shell-pty` launchable → `shell-pty`；否则 herdr 已广播 → `claude-pty`；两者皆无 → 以理由拒绝（`UNSATISFIABLE`），不再回落 print」；**显式 `--driver` 一律照办或以理由拒绝（HTTP 409），绝不静默替换**；`POST /v1/instances` 省略 driver 时回落 `claude-pty`（保持多轮 TUI；且非 shell driver，不改变该请求既有的 agent 审批门槛）。(2) **Node** 请求的 `claude-pty`/`shell-pty` 在构造失败时一律 `instance.create` 带 reason code 拒绝（`unsupported-driver` / `unsupported-kind`），**绝不降级**；拒绝发生在命令被持久接受之前（`DriverRegistry::is_registered`），否则 Hub 会留着一行「运行中」而产品从未启动。(3) **print 只可显式选中**（`--driver claude-print`、web picker 显式选择），并在该处标注为诊断/legacy；web New Session 默认跟随宿主 `driverInventory`（launchable `shell-pty`，否则 `claude-pty`），`prefs` 空值不再预置 print。(4) **名册与实例记录一律记录 Node create 结果里实际跑起来的 driver**（`reconcile_instance_driver`），`remuda watch` 打印该列——Hub 请求值与实跑值不允许再无声分叉。**理由**：2026-09-17 一次 `--harness claude` 派发在 roster 与 instance 上记的是 `claude-pty`，Node 实跑 `claude-print`，journal `source.driverKind` 为 `claude-print`；根因是 Node 侧 `serde_json::from_value` 对 Hub `json!` 生成的显式 `null` 解析失败（`#[serde(default)]` 不覆盖显式 null，`prompt` 在派发路径上恒为 null——brief 走附件），`Err(_)` 分支用硬编码 `claude-print` 默认请求整体替换，且 `apply_spec_launch_fields` 不回读 `kind`/`driver`，于是无人记录这次替换。**影响**：D-028 的「print 按条件退役」提前落地为「显式才可用」；`docs/design/protocol.md` §kind/driver 矩阵与 web picker 文案同步。证据 `evidence/dispatch-driver-1.md`。 | 用户（owner 增补）/ c-dispatchfix | [evidence/dispatch-driver-1.md](./evidence/dispatch-driver-1.md)；D-028、D-028a |
