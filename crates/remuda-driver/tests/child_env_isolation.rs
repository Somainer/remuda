...
/// Loader / proxy / TLS names that must be refused however they arrive.
...
async fn agent_child_is_isolated() {
    ...
    // Sanity: this process really does hold the secrets we expect to be dropped.
    for (name, value) in NODE_SECRETS.iter().chain(HOSTILE) {
        assert_eq!(
    ...
    // S1: no Node secret reaches the child, by name or by value.
    ...
    // S2: no loader, proxy, or TLS override reaches the child.
...
async fn extra_env_cannot_reintroduce_a_denied_name() {
    // A caller-supplied overlay is subject to the same denylist.
