...
### 5.4 disconnected 下拒绝新建会话（D-035 的自然延伸；D-055 修订发送口径）

* `disconnected`（以及 `recovering` 未完成）状态下，手机端**不提供**新建会话
  的提交入口：新建会话 Compose 入口拒绝，不写 outbox、不静默入队
  （`instance.create` 没有客户端幂等键，重试会创建多个会话）。
...
* **D-055 修订（取代 D4 的「不写 outbox」）**：上一段只对**新建会话**与
  **打断**（`instance.cancel`，时效性强且跨 turn 不幂等）继续成立。对已有
  实例的普通 `instance.send`，以及离线时由插话（steer）降级而来的排队发送，
  disconnected 下改为写入本地 outbox：先落盘（客户端生成的 `cmd_`+UUIDv7
  commandId），恢复后以同一 commandId 自动补发，在 Hub/Node commandId 去重
  前提下恰好一次；输入框照常可用，横幅明示「离线 · 可继续输入，恢复后自动
  发送」。超出重试窗口（≤24 h / 20 次）、409 `COMMAND_ID_CONFLICT` 或本机
  存储不可用时，诚实退回「状态待确认」，不假装已发出。详见 D-055。
* 依据：所有者裁定的让步是「手机控制时笔记本必须在线」；对 create 静默入队
  会制造「以为已创建」的错觉（D-035 refuse-never-reroute 的同族原则）。
...
* 不做主动 WSS Ping / TCP keepalive / 接口切换 watcher 的手机侧替代——
  ...
  手机端不依赖它做诚实状态（§5.1）。
* 不做 journal 自动回收/压缩（§3.3）；不做 HA。
* 不做离线 outbox —— **已被 D-055（2026-09-24）修订**：对已有实例的
  `instance.send`（含离线降级的 steer）现在写入客户端 outbox 并同 commandId
  补发；仍然「不做」的是 create 与 cancel 的离线入队（§5.4）。
