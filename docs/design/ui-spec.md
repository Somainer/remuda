...
- **侧栏当前态与已退出分组**：当前 space 与当前会话都用品牌左条 + 底色 + 加粗标题，两者都能一眼看出。每个 space 下的「已退出 (n)」分组默认折叠，行内提供 **恢复**（既有 resume 能力）与 **删除**（`DELETE /v1/instances/{id}`，确认「删除会话及其记录？」；运行中会话先停止，按钮为「停止并删除」）。Hub 尚未提供该路由时按 405/501 探测退化为「从本设备列表隐藏」，提示须说明记录仍在 Hub 上。
