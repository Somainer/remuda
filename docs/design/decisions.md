...
**(2) 开关（Hub 进程环境变量，无 wire/create-spec 字段）**

实现位置：`crates/remuda-hub/src/interactions.rs:43-104`。开关只在 **Hub 进程**读取，**不**经 driver→Node→Hub 的 create spec 传递，driver/Node 无独立设置；create spec 与 wire/schema 上都没有该字段。

- **全局开关** `REMUDA_DELEGATED_DECISIONS`（truthy=`1/true/on/yes/enabled` 开，缺省关）。
- **per-project 覆盖名单**（逗号分隔 project id）：`REMUDA_DELEGATED_DECISIONS_FORCE_ON`（这些项目强制开）、`REMUDA_DELEGATED_DECISIONS_FORCE_OFF`（强制关）。
- **优先级**（`resolve_delegated_decisions`，`interactions.rs:71-87`）：项目 id 在 FORCE_OFF → 关；在 FORCE_ON → 开；否则取全局。FORCE_OFF 先判（项目同时上两单时 fail-closed 为关）。项目之外（`project_id=None`）只看全局。
- 进程内另有一个测试用全局 override（`interactions.rs:47` 的 `DELEGATED_GLOBAL_OVERRIDE` + `delegated_decisions_test_support::set_global_override`，`interactions.rs:915-919`），仅测试替换环境变量用，不暴露给 wire。
- 旧行为字节不变（缺省关）；Agent 不可见时产物（journal event、list 投影）与开关无关，只是 Hub 路由不放行。
