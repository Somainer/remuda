...
- **E2E, hub-live** (`playwright.hub.config.ts`): full suite post-merge under `flock` — **33 passed, 14 skipped (batch B's conditional mock/hub cases), 0 failed**; the passkey spec exercises the kept Passkeys section end-to-end (register → logout → passkey login), and pairing exercises the paired-devices/pair-code rows.
- A handful of mock-mode specs fail on this shared devbox identically on clean `origin/main` (verified by re-running them in a baseline worktree at the post-merge main: `agent-board`, `session-structured`, `spaces`, `ux-status` login, and the two deterministic `grok pty` / `shell-pty` cases) — host contention and documented baseline flakes, not regressions from this batch.
