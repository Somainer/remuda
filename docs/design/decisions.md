...
| 相关 | D-021（secret 只留一台机器）、D-031（禁隧道）、D-035（禁止静默替换，记录实跑值）、D-048（`api.*` 流类）、api-routing 计划（`briefs/plans/api-routing.md`） §B.0/§B.1/§B.5/§B.9 + Amendment A1、[protocol.md](./protocol.md) §4.4/§7.6 |
...
`transport_limits_written_before_d048_still_parse`）。行为与 CLI/web 面是
tasks 2–6。
...
| 相关 | D-047、D-027a（`object.pull` 先例）、[protocol.md](./protocol.md) §7.4/§7.6、api-routing 计划（`briefs/plans/api-routing.md`） §B.2 |
