...
- **侧栏强当前态**：当前 space 与当前会话都用品牌左条 + 底色 + 加粗标题。
  ...
  该路由由 x-ttyhub 实现：终态实例直接删除，运行中实例返回 409，需 `?force=1`
  由 Hub 先停止再删除——因此「停止并删除」只发这一个请求，客户端**不再**自行
  先 close；重复删除返回 404，按幂等成功处理。响应 `nodePurge` 非 `purged` 时
  Hub 记录已删、主机侧数据待清理，提示如实说明。对更早的 Hub 仍按 405/501
  退化为「从本设备列表隐藏」，并说明记录仍在 Hub 上，不谎称已删除。
