...
| 被拒后焦点 | `SettingsPage.test.tsx`「returns focus to the committed radio…」：写入抛错时方向键选中被回滚，焦点回到已提交项（`tabIndex=0` 且为 `activeElement`） | 通过 |
...
| 字体晚到 | `font-swap.hub.spec.ts`：woff2 延迟 800ms，每条先做一次无换字体对照（字体已缓存）再做换字体恢复。断言：换字体恢复距保存位置不超过对照的误差 + 4px（换字体不引入额外偏移）、字体换入前后位移 ≤ 4px、行不重叠。短会话：`saved=249.375 control=344.375 beforeSwap=348.375 settled=348.375`（对照误差 95，换字体 99，多 4px，换入时位移 0）。长会话（2400 行突发 + 代码回复 + 24 行，超过 Hub 2000 行尾窗，断言 `fromSeq>1` 且 `reachedAfterSeq=false`，即只重放有界窗口）：`saved=249.25 control=227.66 beforeSwap=248.66 settled=248.66`（对照误差 21.6，换字体 0.6，换入时位移 0）。两条连续各跑 2 遍数值一致；390 置底会话换字后仍置底 | 通过 |
