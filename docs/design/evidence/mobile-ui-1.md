...
| 文件 | 变化 |
|---|---|
| `docs/design/ui-spec.md` | v0.2.2 changelog；§1.1 审批行补 compact 落点；§1.2 路由表（`/m`、`/m/inbox` 两行 + `/` 改按视口落点 + `/sessions`、`/approvals` 行补 compact 重定向）+ 表后重定向层表；§1.3 手机线框重画为两套铬（会话路由主行含状态点）+ 「会话路由 compact 不渲染底部导航栏」规则；§1.4 新增 tabs 行例外；§4.5 补 badge 与权限横幅位置、PWA 关闭态；§4.6 `start_url` 改 `/`；新增 §4.7、§4.8 |
| `docs/design/decisions.md` | 顶部索引表追加 D-049 一行（D-048 之后）；文末追加 `## D-049` 正文段（日期/状态/相关 + 背景/决策/由谁/依据，同 D-034 以来各 ADR 的形状） |
| `docs/design/evidence/mobile-ui-1.md` | 本文件 |

## 1.1 handback（2026-09-19）五处修订

coordinator 在 `1abdfb84` 上 review 后要求的 docs-only 修订，单 commit 叠在其上：

1. `decisions.md` 补 `## D-049` 正文段（背景写明选项 1「只做共享路由 compact」与选项 3「独立客户端」为何被拒，决策指向 ui-spec §4.7/§4.8 而不重抄条文）。
2. `ui-spec.md` §1.4 在 D-040 chips 例外之后追加「compact 下的 tabs 行例外（D-049）」，消除「约 400px 显示可横向滚动的 space chips 与 tabs」对 tabs 的无条件要求与 §1.3 新规则的矛盾。
3. 裁决：compact 主行恒为 返回 / space 芯片 / 标题 / 状态点 / 分段 / Stop / ⋯；**host 芯片与 cost 仅在 compact 折进「运行详情」**（space 芯片已点名主机与项目），D-040 的桌面主行规则不变。§1.3 线框补状态点、§4.7 截断梯补该例外、D-049 索引行与正文段均记录为 compact-only。
4. §4.7 依据行把 DEV SERVERS / Kill 端口旁栏的「整体不抄」从 §9 移到 §11.5 引用下（§9 只说「不要先做」）。
5. 终端段预算算式写成「顶栏 52 + 触发行 20」（不再写易被读成第二种顶栏高度的「顶栏 72」）；§1.1 审批行的手机落点补「compact 下即 /m/inbox」；线框的 52px 注记移入框内、右框右边框闭合。
...
| **新文** | 线框重画为**两套铬**：首页级屏（`/m`、`/m/inbox`）保留 64px app 底栏；会话路由（`/s/:id*`）只有 52px 顶栏 + 正文 + composer/键盘条，**无 app 底栏**，顶栏框内画出 `← [space] 标题 ● [终端|结构] ■ ⋯ 52px` 与「▸ 运行详情（host/cost 折入）20px」（含状态点，handback 第 3 项）。线框下新增规则段：会话路由 compact 不渲染 app 底栏（`Shell.tsx` 的 `.bar`，今天在 `:310`）也不渲染 SpaceTabs 行（`:306`）；回列表靠 `SessionPage.tsx:239` 返回键（44px 热区，D-039）；切空间/切 tab 靠 D-040 的单芯片抽屉（`spaces-drawer-open`）与 Jump To，能力不降级 |
| **处理方式** | 旧的「映射规则」表与 Paseo 段落全部保留；新规则是**在手机小节内增补**，桌面三栏线框一个字没动。底栏文案「收件箱(n)」与 §1.1 表的「审批」之差：§1.1 手机落点单元格补注「compact 下即 `/m/inbox`，D-049」，§4.7 不再携带第二个答案 |
...
| **新文** | 新增整节，五块：(1) **受限 `/m` 树**（只含 home / inbox / Jump To sheet / phone 底栏的一张表）+ 「会话本体永远是共享 `/s/:id`、不分叉」+ 引报告 §9 SOTA 一句话；(2) 指向 §1.2 的重定向与深链三连；(3) **可测量铬预算**——最多一条顶栏（`--top-mobile`，`tokens.css:79`）+ 一条底栏（`--bar`，`tokens.css:77`，+ `--safe-bottom`，`:80`）；会话路由无 app 底栏/无 SpaceTabs 行；**正文（`session-body`，`SessionPage.tsx:416`，终端段测 xterm 容器）composer 收起、无软键盘时 ≥ 60% 视口高**，并给出 390×844 的余量算式（结构与终端都按 安全区 47 + 顶栏 52 + 触发行 20 起算，≈ 75% / 71%，60% 是下限不是目标）；(4) **截断优先级**：标题 → space 芯片退首字母 → 状态文字（只留点），状态点本身恒在主行；其下钉 **compact-only 例外**：host 芯片与 cost 折进「运行详情」（D-040 桌面主行规则不变），主行恒为 返回 / space 芯片 / 标题 / 状态点 / 分段 / Stop / ⋯；**`终端\|结构` 分段（`SessionPage.tsx:258`）与 Stop（`:329`）永不截断、永不进 ⋯**（D-040）；(5) 依据清单逐小节点名报告——DEV SERVERS/Kill 旁栏挂 §11.5「整体不抄」（§9 只说不要先做，handback 第 4 项），并钉 M1/M2 边界 |
| **为何这些数字是「规格」而不是实施细节** | 报告 §2.1 的观察是「垂直铬叠五层，正文只剩中间一小块」；没有可测量下限，每个实施单都会自行其是。60% + 两条栏 + 截断顺序让任务 5（m-sessionfold）的几何验收直接引用本节，不需要再发明口径。host/cost 的折入口径来自 handback 裁决：space 芯片已回答主机与项目，cost 在「运行详情」内仍可读，桌面规则不触碰 |
| **§1.4 tabs 例外（handback 第 2 项）** | 同节新增的规则若不回头限定 §1.4，§1.4 原句「约 400px 手机上显示可横向滚动的 space chips 与 tabs」会继续无条件要求 tabs 行——与新规则直接矛盾。处理同 D-040 chips 例外：原句不动，在其后追加一条同形状子弹，把「与 tabs」半句限定到列表路由；对照表见本文件 §4 |
...
| **D-040** | `/s/:id` compact：chips 折成**单枚**当前 space 芯片入顶栏；诊断进「运行详情」；分段与 Stop 永不进 ⋯；**桌面**主行必须可见 host 芯片与 cost | §1.3 + §4.7 规定会话路由 compact **既不渲染 app 底栏、也不渲染 SpaceTabs 行**；截断优先级把 space 芯片排在标题之后、状态文字之前；分段/Stop 的「永不截断、永不进 ⋯」原文照抄并补了挂载位置（`SessionPage.tsx:258` / `:329`）；space 芯片点开仍是 D-040 的同一个抽屉（`spaces-drawer-open`）。**handback 裁决后的 compact-only 例外**：compact 主行恒为 返回 / space 芯片 / 标题 / **状态点** / 分段 / Stop / ⋯，而 **host 芯片与 cost 折进第二行「运行详情」disclosure**（space 芯片已点名主机与项目，cost 展开仍可读，不删不猜）；D-040 的**桌面**主行规则一字不动 | **同向增补（additive），不是改写**：D-040 解决「chips 行折成单芯片」，本批解决「单芯片之外那条 SpaceTabs 行与 app 底栏也不该占正文」，host/cost 折入是显式标注的 **compact-only** 边界、桌面规则不触碰；D-040 允许的抽屉、状态点、分段、Stop 一条不少，切空间/切 tab 由抽屉 + Jump To 兜底并被明文写成「能力不降级」。D-040 的文字无需修改即与本批兼容 |
...
| **§1.4 原句（D-024 区域，不在 D-038…D-042 内，handback 第 2 项）** | §1.4 快捷键句原文：「约 400px 手机上显示可横向滚动的 space chips **与 tabs**，左侧面板通过抽屉访问」 | 该句对 **chips** 的应用范围已由 D-040 例外限定；本批在其后追加一条同形状的 **「compact 下的 tabs 行例外（D-049）」**：该句对 tabs 的要求只在列表路由成立，`/s/:instanceId*` compact 不渲染 tabs 行，切 tab 由单芯片抽屉与 Jump To 承担，列表路由不变 | **限定作用域，不改原句**：与 D-040 chips 例外同一处理手法——原句保留以不与 `spaces-1.md` / `tabs-1.md` 验收脱节，新子弹显式覆盖「与 tabs」半句；否则该句与 §1.3「会话路由 compact 不渲染 SpaceTabs 行」直接矛盾 |

结论：五条 ADR 与 §1.4 的 D-024 旧句均无需修订原文；本批是在 D-040 的单芯片规则**之上**再收两条（SpaceTabs 行、app 底栏）并加一条 compact-only 的 host/cost 折入，叠加后 `/s/:id` compact 的顶栏能力仍全部可达。

## 5. 引用的 file:line 核对（全部以 `git show origin/main:<file>` 验证）

规格正文出现的每一条代码 file:line 都用 `git show origin/main:<file>` 逐条验证；首批编辑前基线为 `402c3670`，handback 修订 rebase 到 `5bdf0052` 后 19 条全部重新核对，内容未漂移；本分支不改代码，行号不会在本批内漂移。
